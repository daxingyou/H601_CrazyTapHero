...
//            Main.singleton.mainMenuVM.currentPage = PageName.MagicWeapon;
            ...
            if(this.artifactGroup.numChildren == 0) {
                this.mWBuyItem = new MallItemVM(this.artifactGroup,() => {//创建商城购买按钮.
                ...
                for(var i = 0;i < mWDatas.length;i++) {
                    var item: MallItemVM = new MallItemVM(this.artifactGroup,(_data: Model.MagicWeaponData) => {
                        Model.AudioService.Shared().PlaySound("YX-004_mp3");
                ...
                for(var i = 0;i < sDatas.length; i++) {
                    var shopItem: ShopItemVM = new ShopItemVM(this.shopGroup,(_data:Model.ShopData,_isFree?:boolean)=>{
                        ...
                        if(_data.Type==Model.ShopType.SHOP_TYPE_YB){
                            //中间件支付接口 by cai_haotian 2016.3.29
                            var item: Item = {
                                "id": _data.st.id,
                                "name": _data.st.name,
                                "price": _data.st.costRmb,
                                "count": 1,
                                "currency": "CUR_CNY"
                            };
                            DCAgent.onPayment({ amount: _data.st.costRmb,currencyType: "CUR_CNY",payType: "",iapid: "",orderId: _data.st.id });
                            lwsdk.pay(item,() => {
