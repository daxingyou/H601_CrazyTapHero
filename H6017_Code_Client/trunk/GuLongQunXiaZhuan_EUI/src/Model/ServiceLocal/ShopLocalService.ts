module Model{
    ...
    export class ShopLocalService {
      ...
      public static upGradeShop(_data: ShopData,_isFree:boolean,_onCallBack:Function){
    	...
    	public static flyBox(_gold:number,_timer:number){
            var flyBox = new ViewModel.FlyBox(Main.singleton.mainMenuVM,() => {
            })
            flyBox.showFun();
            ...
            timer.addEventListener(egret.TimerEvent.TIMER_COMPLETE,() => {
                flyBox.dropDownFun(() => {
                    ...
                    Main.singleton.mainMenuVM.flyBoxDescription.text = "获得:" + MainLocalService.toUnitConversion(Math.round(_gold / _timer)) + "铜币";
                    egret.setTimeout(() => {
                        Main.singleton.mainMenuVM.flyBoxGroup.visible = false;
                    },this,1000);
                    PlayerLocalService.PlayerData.AddGold = Math.round(_gold / _timer);
                });
            },this)
            timer.start();
            flyBox.clickFun(() => {
