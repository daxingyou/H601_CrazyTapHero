    ...
    var ShopLocalService = (function () {
       ...
       * @商城5000元宝购买金币=100元宝购钱*1000000
        ...
        ShopLocalService.addMoney = function (_gold, _timer) {
            for (var i = 0; i < _timer; i++) {
                ShopLocalService.flyBox(_gold, _timer);
            }
        ...
        ShopLocalService.flyBox = function (_gold, _timer) {
            var _this = this;
            var flyBox = new ViewModel.FlyBox(Main.singleton.mainMenuVM, function () {
            });
            flyBox.showFun();
            var timer = new egret.Timer(3000, 1);
            timer.addEventListener(egret.TimerEvent.TIMER_COMPLETE, function () {
                flyBox.dropDownFun(function () {
                    Main.singleton.mainMenuVM.flyBoxGroup.visible = true;
                    Main.singleton.mainMenuVM.flyBoxIcon.source = "";
                    Main.singleton.mainMenuVM.flyBoxDescription.text = "获得:" + Model.MainLocalService.toUnitConversion(Math.round(_gold / _timer)) + "铜币";
                    egret.setTimeout(function () {
                        Main.singleton.mainMenuVM.flyBoxGroup.visible = false;
                    }, _this, 1000);
                    Model.PlayerLocalService.PlayerData.AddGold = Math.round(_gold / _timer);
                });
            }, this);
            timer.start();
            flyBox.clickFun(function () {
                timer.stop();
                Main.singleton.mainMenuVM.flyBoxGroup.visible = true;
                Main.singleton.mainMenuVM.flyBoxIcon.source = "";
                Main.singleton.mainMenuVM.flyBoxDescription.text = "获得:" + Model.MainLocalService.toUnitConversion(Math.round(_gold / _timer)) + "铜币";
